...
class PushReward:
    ...
    def __init__(self):
        ...
        self.xmin = np.array([-5., -5., -10., -10., 2., 0., -5., -5., -10., -10., 2., 0., -5., -5.])
        self.xmax = np.array(
    ...
    def f_max(self):
        ...
        return np.linalg.norm(np.array(self.gxy) - np.array(self.sxy)) \
            + np.linalg.norm(np.array(self.gxy2) - np.array(self.sxy2)) # type: ignore

    def __call__(self, argv, gui=False):
        # returns the reward of pushing two objects with two robots
        argv = np.array(argv) * (self.xmax - self.xmin) + self.xmin
        rx = float(argv[0])
        ry = float(argv[1])
        xvel = float(argv[2])
        yvel = float(argv[3])
        ...
        world = b2WorldInterface(do_gui=gui)
...
def get_push_func():
    f = PushReward().__call__
    return f, 14


def main():
    f, nn = get_push_func()
    x = np.random.uniform(size=nn)
